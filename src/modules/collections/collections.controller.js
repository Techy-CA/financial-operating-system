  ...
  _render(){
    ...
    Router.render(`
      <div class="page-header">
        ...
        <div class="page-header-actions">
          <button class="btn btn-primary btn-sm" id="btn-remind-all" onclick="CollectionsPage.remindAll()">
            📧 Send all reminders ${withEmail>0?`(${withEmail})`:''} 
          </button>
        </div>
      </div>

      ${noEmail>0?`<div style="background:var(--color-warning-light);border:1px solid var(--color-warning-mid);border-radius:10px;padding:12px 16px;margin-bottom:16px;font-size:13px;color:var(--color-warning-text);">
        ⚠️ <strong>${noEmail} customer${noEmail!==1?'s':''}</strong> don't have email addresses — reminders won't be sent to them. <a href="#/customers" style="color:var(--color-warning-text);font-weight:700;text-decoration:underline;">Add emails →</a>
      </div>`:''}
      ...
      <div class="card">
        ...
        ${list.length===0?`<div class="empty-state" style="padding:48px;"><div class="empty-state-icon">✅</div><h3>All clear!</h3><p>No outstanding receivables.</p></div>`:
        `<div class="table-wrapper"><table class="data-table">
          ...
          <tbody>
            ${list.map(inv=>{
              ...
              else if(rem<=7){daysTxt=`Due in ${rem}d ⚠️`;daysColor='var(--color-warning)';}
              ...
              return `<tr>
                <td>
                  ...
                  <div style="font-size:11px;color:${hasEmail?'var(--color-success)':'var(--color-danger)'};">${hasEmail?`📧 ${inv.customerEmail}`:'⚠ No email'}</div>
                </td>
                ...
                <td class="col-actions"><div class="row-actions">
                  ...
                  <a href="#/invoices/${inv.id}" class="btn btn-ghost btn-icon btn-sm">👁</a>
                </div></td>
              </tr>`;
            }).join('')}
          </tbody>
